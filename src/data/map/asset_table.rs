...
impl AssetTable {
    pub fn dump(self, mod_dir: &ModDir) -> Result<(), std::io::Error> {
        ...
        for asset in self.assets {
            match asset.data {
                AssetData::Background { width, height, raster, palette } => {
...
impl RomRead for Asset {
    fn read(rom: &mut Rom) -> Result<Self, ReadError> {
        ...
        Ok(Asset {
            ...
            name, data_offset, compressed_size, decompressed_size,
        })
    }
}

fn read_u16(bytes: &[u8]) -> u16 {
    u16::from_be_bytes(bytes.try_into().expect("unexpected end-of-data while reading u16"))
}

fn read_u32(bytes: &[u8]) -> u32 {
    u32::from_be_bytes(bytes.try_into().expect("unexpected end-of-data while reading u32"))
}
