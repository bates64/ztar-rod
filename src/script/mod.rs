...
/// Represents a variable type known at compile-time.
#[derive(Debug)]
pub enum Type {
    Any,
    Int,
...
impl Type {
    /// Provides a sensible default `Value` for the type. Usually this is zero,
    /// or in the case of callable `Function` and `Asm` types a no-op callable.
    ...
    fn try_from(pair: Pair<'a, Rule>) -> Result<Self, Error> {
        Ok(match pair.as_rule() {
            Rule::ty => match pair.as_str() {
                ...
                "float" => Type::Float,
                "bool"  => Type::Bool,
                _       => bail_at!(pair.as_span(), "unimplemented type"),
            },
...
/// A function or asm signature. Defines return type(s) and arg type(s), if any.
#[derive(Debug)]
...
impl Signature {
    pub fn new(args: Vec<Type>, returns: Option<Type>) -> Self {
    ...
    fn try_from(pair: Pair<'a, Rule>) -> Result<Self, Error> {
        Ok(match pair.as_rule() {
            Rule::function => {
                ...
                let name = pairs.next().unwrap().try_into()?;

                let mut arg_names = Vec::new();
                let mut arg_types = Vec::new();
                for (ty, id) in pairs.next().unwrap().into_inner().tuples() {
                    arg_names.push(id.try_into()?);
                    arg_types.push(ty.try_into()?);
                }

                // TODO return types in grammar
                let ret_type = None;

                Function {
                    ident: name,
                    offset: None,
                    signature: Signature::new(arg_types, ret_type),
                    arg_names: arg_names,
                    stmts: pairs.next().unwrap().into_inner().map(|pair| pair.try_into()).collect::<Result<Vec<_>, _>>()?,
                }
            },
...
impl TryFrom<&bc::Bytecode> for Function {
    type Error = Error;
    fn try_from(bc: &bc::Bytecode) -> Result<Function, Error> {
        let source = format!("{}", bc);
            ...
            Rule::stmt => {
                ...
                match pair.as_rule() {
                    ...
                    */
                    ...
                    // Return statement. Providing a return value is optional.
                    Rule::return_stmt => Statement::Return {
                        expr: match pair.into_inner().next() {
                            Some(expr) => Some(expr.try_into()?),
                            None       => None,
                        },
                    },
